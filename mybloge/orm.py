...
# 创建sql连接池
async def create_mysql(loop, **kwargs):
...
class StringField(Field):
    def __init__(self, name=None, column_type='varchar(100)', primary_key=False, default=''):
...
class DatetimeField(Field):
    def __init__(self, name=None, column_type='datetime', primary_key=False, default='NULL'):
        super().__init__(name, column_type, primary_key, default)
    ...
    def __new__(cls, name, bases, attrs):
        ...
        for k, v in attrs.items():
            if isinstance(v, Field):
                ...
                mapping[k] = v
                if v.primary_key:
                    logging.info('Find primarykey %s' % k)
                    if primarykey:
        ...
        attrs['__insert__'] = 'INSERT INTO `%s` (`%s`,%s) VALUES (%s)' % (
            tablename, primarykey, ','.join(escaped_fields), creatr_args_string(len(escaped_fields) + 1))
...
class Model(dict, metaclass=ModelMeatclass):
    ...
    async def DELETE(cls,pk):
        sql = cls.__delete__
        args = [pk]
        rows = await execute(sql,args=args)
        if rows!=1:
            logging.info('failed delete by primarykey %s' % pk)
            return False
        return True
